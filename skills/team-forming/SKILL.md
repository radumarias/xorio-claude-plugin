...
| MAX | top tier + `effort="max"` + prompt begins with "Ultrathink — use maximum thinking." | the architect/planner, the one-shot plan-reviewer (plan-review gate), and the verification roles — `code-reviewer` and `qa-tester` — (pinned whenever present); otherwise only when explicitly warranted |
...
**`--fable` override.** By default the top tier resolves to `opus` — Fable/Mythos are access-gated, so they are deliberately *not* the default. Passing `--fable` resolves the top tier to `model="fable"` for every top-tier (HIGH and MAX) role — an explicit opt-in for accounts that can actually spawn Fable (the most capable, most expensive model). It does **not** touch `haiku`/`sonnet` roles: cheaper roles stay on their tiers; only the reasoning-critical best-model roles (architect/planner, plan-reviewer, `code-reviewer`, `qa-tester`, and any role derived at HIGH/MAX) move to Fable. MAX roles keep their `effort="max"` + `Ultrathink` prefix on top of the Fable model. Because `--fable` is an explicit request, if a Fable spawn returns "`fable` is currently unavailable", warn the user and fall back to `opus` (see **Troubleshooting**) rather than silently proceeding.
...
For MAX-tier roles, prefix the prompt with `Ultrathink — use maximum thinking.` and spawn them with `effort="max"` (and `model="fable"` when `--fable` is set).
...
2. For each remaining role: `Agent(team_name="{team-slug}", name="{role-name}", model="{haiku|sonnet|opus|fable}", prompt="{prompt from Build Team Prompts}")` — spawn the whole batch in one message so members run concurrently. The `code-reviewer` and `qa-tester` roles are MAX tier, so they spawn with `effort="max"` and their prompts take the `Ultrathink — use maximum thinking.` prefix; with `--fable` their `model` is `fable` instead of `opus`.
