...
- `team-forming` — composes and spawns an agent team for one task-group. A MAX-tier architect/planner (latest Opus + ultrathink today) runs first to design the work and recommend the team composition, then stays on as a standing advisor that teammates consult (its design doc is also persisted to `~/.claude/teams-state/` so clarification survives the architect); an optional Phase −1 recon (default-on, `--no-explore` to skip, auto-skipped when there is no architect or `feature-dev` is absent) first runs feature-dev's `code-explorer` as a one-shot to hand the architect a distilled codebase map so its MAX budget goes to design not discovery; an independent MAX-tier reviewer then gates the plan (a default-on Adversarial Plan-Review Gate) before the team forms; a default-on Human Plan-Approval Checkpoint then lets the user Approve/Revise/Abort the accepted plan (skip with `--no-confirm-plan`; auto-skipped when there is no architect) before wave 1; the cascade then derives any remaining roles, builds disposition-based prompts, and spawns via `TeamCreate`/`Agent`. The verification roles — a `code-reviewer` (static) and a `qa-tester` (behavioural, included when the deliverable is runnable), split so each lens gets full attention — are likewise pinned to MAX. For task-groups that build or reshape visual UI, the architect composes a `frontend-developer` role (a developer specialisation) that invokes the `frontend-design` skill for aesthetic direction *before* writing components — HIGH tier when the UI is design-heavy; pure frontend logic/plumbing uses a generic developer instead. The top tier is defined once in the skill's **Model tiers** table and resolves to the most capable model available (`opus` today), so no edit is needed when a stronger model ships; MAX roles spawn at `effort="max"`, and the opt-in `--fable` flag resolves the top tier to `fable` (instead of `opus`) for the best-model roles (architect, plan-reviewer, code-reviewer, qa-tester). State lives under `~/.claude/teams-state/` (independent of plugin location). Trigger phrases: "form team", "create team", "compose team".
...
- `PreToolUse(Bash|Read|Grep)` — runs `hooks/block-secret-file-reads.mjs` (invoked as `node ${CLAUDE_PLUGIN_ROOT}/hooks/...`), a deny-only guard for reading secret / private-data files (`.env`, `.secrets`, key/cert files, `id_rsa*`, `credentials`, `.pgpass`, `.netrc`). It covers **all three** content-reading vectors with one script: the *shell* vector (Bash — inspects `tool_input.command`, with quoted segments stripped so a mere mention isn't tripped), the *tool* vector (Read — inspects `tool_input.file_path`), and the *search* vector (Grep — whose `output_mode:"content"` returns file lines, so it inspects the path-like `tool_input.path` / `tool_input.glob`, never the search `pattern`). It emits a `deny` decision only on a bare protected-file reference; `*.example` templates stay allowed. Stdlib-only Node, no config, never reads file contents.
